...
describe 'fhgfs' do
  ...
  context 'osfamily => Foo' do
    let(:facts) { default_facts.merge({ :osfamily => "Foo" }) }
  
    it { expect { should create_class('fhgfs') }.to raise_error(Puppet::Error, /Unsupported osfamily/) }
    #it { should_not contain_file('/etc/pki/rpm-gpg/RPM-GPG-KEY-fhgfs') }
    #it { should_not contain_gpg_key('fhgfs') }
    #it { should_not contain_yumrepo('fhgfs') }
  end
end
