shared_examples_for 'beegfs::client::config' do
  ...
  it do
    is_expected.to contain_file('/etc/beegfs/beegfs-client.conf').with(ensure: 'present',
                                                                       owner: 'root',
                                                                       group: 'root',
                                                                       mode: '0644')
  end
  ...
  it do
    is_expected.to contain_file('/etc/beegfs/interfaces.client').with(ensure: 'absent',
                                                                      content: %r{^$},
                                                                      owner: 'root',
                                                                      group: 'root',
                                                                      mode: '0644')
  ...
  it do
    is_expected.to contain_file('/etc/beegfs/netfilter.client').with(ensure: 'absent',
                                                                     content: %r{^$},
                                                                     owner: 'root',
                                                                     group: 'root',
                                                                     mode: '0644')
  ...
  it do
    is_expected.to contain_file('/etc/beegfs/beegfs-helperd.conf').with(ensure: 'present',
                                                                        owner: 'root',
                                                                        group: 'root',
                                                                        mode: '0644')
  end
  ...
  it do
    is_expected.to contain_exec('mkdir -p /mnt/beegfs').with(path: '/usr/bin:/bin:/usr/sbin:/sbin',
                                                             creates: '/mnt/beegfs')
  end

  it do
    is_expected.to contain_file('/etc/beegfs/beegfs-mounts.conf').with(ensure: 'present',
                                                                       owner: 'root',
                                                                       group: 'root',
                                                                       mode: '0644')
  end
  ...
  it do
    is_expected.to contain_file('/etc/beegfs/beegfs-client-autobuild.conf').with(ensure: 'present',
                                                                                 owner: 'root',
                                                                                 group: 'root',
                                                                                 mode: '0644')
  end
  ...
  it do
    is_expected.to contain_file_line('beegfs-client-autobuild buildArgs').with(ensure: 'present',
  ...
  it do
    is_expected.to contain_file_line('beegfs-client-autobuild buildEnabled').with(ensure: 'present',
                                                                                  path: '/etc/beegfs/beegfs-client-autobuild.conf',
  ...
  context 'when client_conn_interfaces => ["eth0"]' do
    ...
    it { is_expected.to contain_file('/etc/beegfs/interfaces.client').with_ensure('present') }
  ...
  context 'when client_conn_net_filters => ["192.168.1.0/24"]' do
    ...
    it { is_expected.to contain_file('/etc/beegfs/netfilter.client').with_ensure('present') }
  ...
  context 'when client_service_autorestart => false' do
    ...
    it { is_expected.to contain_file('/etc/beegfs/beegfs-client-autobuild.conf').without_notify }
  end

  context 'when utils_only => true' do
    let(:params) { { utils_only: true } }

    it { is_expected.to contain_file('/etc/beegfs/beegfs-client.conf') }
    it { is_expected.to contain_file('/etc/beegfs/interfaces.client') }
    it { is_expected.to contain_file('/etc/beegfs/netfilter.client') }
    it { is_expected.not_to contain_file('/etc/beegfs/beegfs-helperd.conf') }
    it { is_expected.not_to contain_file('/etc/beegfs/beegfs-mounts.conf') }
    it { is_expected.not_to contain_file('/etc/beegfs/beegfs-client-autobuild.conf') }
  end
