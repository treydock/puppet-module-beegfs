shared_context 'beegfs::meta::config' do
  ...
  it { is_expected.not_to contain_file('beegfs-storeMetaDirectory') }

  it do
    is_expected.to contain_file('/etc/beegfs/beegfs-meta.conf').with(ensure: 'present',
                                                                     owner: 'root',
                                                                     group: 'root',
                                                                     mode: '0644')
  end
  ...
  it do
    is_expected.to contain_file('/etc/beegfs/interfaces.meta').with(ensure: 'absent',
                                                                    content: %r{^$},
                                                                    owner: 'root',
                                                                    group: 'root',
                                                                    mode: '0644')
  ...
  it do
    is_expected.to contain_file('/etc/beegfs/netfilter.meta').with(ensure: 'absent',
                                                                   content: %r{^$},
                                                                   owner: 'root',
                                                                   group: 'root',
                                                                   mode: '0644')
  ...
  context 'when meta_conn_interfaces => ["eth0"]' do
    ...
    it { is_expected.to contain_file('/etc/beegfs/interfaces.meta').with_ensure('present') }
  ...
  context 'when meta_conn_net_filters => ["192.168.1.0/24"]' do
    ...
    it { is_expected.to contain_file('/etc/beegfs/netfilter.meta').with_ensure('present') }
  ...
  context 'when meta_store_directory => "/beegfs/meta"' do
    ...
    it do
      is_expected.to contain_file('beegfs-storeMetaDirectory').with(ensure: 'directory',
                                                                    path: '/beegfs/meta')
    end
