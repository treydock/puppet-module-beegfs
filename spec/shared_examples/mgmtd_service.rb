shared_examples_for 'beegfs::mgmtd::service' do
  ...
  it do
    should contain_service('beegfs-mgmtd').only_with({
      :ensure       => 'running',
      :enable       => 'true',
      :name         => 'beegfs-mgmtd',
      :hasstatus    => 'true',
  ...
  context 'with mgmtd_service_ensure => "running"' do
    ...
    it { should contain_service('beegfs-mgmtd').with_ensure('stopped') }
  end

  context 'with mgmtd_service_enable => false' do
    let(:params) {{ :mgmtd => true, :mgmtd_service_enable => false }}
    it { should contain_service('beegfs-mgmtd').with_enable('false') }
  end
  ...
  context 'with mgmtd_manage_service => false' do
    ...
    it { should_not contain_service('beegfs-mgmtd') }
  end
end
